...
def make_xml(df: pd.DataFrame, descr_df: pd.DataFrame, dst_filename: str):
    ...
    for instruction_name in instruction_list:
        description = descr_df[descr_df["instruction"] == instruction_name][
            "description"
        ]
        if not description.empty:
            ...
            description = ""
        instruction_elem = ET.Element(
            "Instruction", name=instruction_name.lower(), summary=description
        )
        ...
        for _, row in dff.iterrows():
            ...
            instructionform_elem = ET.Element(
                "InstructionForm", form=instr_form, z80name=instruction_name.lower()
            )
            # opcodes
            ...
            if clean_str(row["timing_z80"]) != "nan":
                instructionform_elem.append(
                    ET.Element("TimingZ80", value=clean_str(row["timing_z80"]))
                )
            if clean_str(row["timing_z80_m1"]) != "nan":
                instructionform_elem.append(
                    ET.Element("TimingZ80M1", value=clean_str(row["timing_z80_m1"]))
                )
            if clean_str(row["timing_r800"]) != "nan":
                instructionform_elem.append(
                    ET.Element("TimingR800", value=clean_str(row["timing_r800"]))
                )
            if clean_str(row["timing_r800_wait"]) != "nan":
                instructionform_elem.append(
                    ET.Element(
                        "TimingR800Wait", value=clean_str(row["timing_r800_wait"])
                    )
                )
