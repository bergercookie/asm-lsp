...
// YAML preprocessing function to fix field names with parentheses
fn preprocess_yaml_content(yaml_content: &str) -> String {
    yaml_content
...
pub fn process_riscv_directory(input_dir: &Path, output_file: &Path) -> Result<()> {
    ...
    fs::write(output_file, json_data).context("Failed to write output file")?;

    println!(
        "Successfully wrote consolidated data to: {}",
        output_file.display()
    );
